...
# Función para conectarse a la base de datos de manera asíncrona
...
async def save_customer_id_to_db(username, customer_id):
    conn = await get_db_connection()  # Obtener conexión
    try:
        async with conn.cursor() as cursor:
            ...
            if result:
                # Si el usuario existe, actualizar el customer_id
                await cursor.execute(
                    "UPDATE users SET customer_id = %s WHERE username = %s",
                    (customer_id, username)
                )
            else:
                # Si no existe, insertar el nuevo usuario con customer_id
                await cursor.execute(
...
async def get_logged_in_user():
    ...
    try:
        # Verificar si hay múltiples clientes con el mismo username
        ...
        if matching_customers:
            ...
            # Seleccionar el customer más reciente (puedes cambiar la lógica según sea necesario)
        ...
        # Guardar el customer_id principal en la base de datos
        await save_customer_id_to_db(username, customer["id"])

        # Crear la sesión de Checkout con el customer_id y username en metadata
        session = stripe.checkout.Session.create(
            payment_method_types=["card"],
            customer=customer["id"],  # Usamos el customer_id creado o encontrado
            line_items=[  # Aquí debes incluir el precio de tu plan
                {
                    'price_data': {
                        'currency': 'usd',
                        'product_data': {
                        ...
                        'recurring': {
                            'interval': 'month',  # O 'year' dependiendo de la periodicidad
                        },
                        'unit_amount': 2000,  # Asegúrate de usar el precio correcto (en centavos, por ejemplo, 20.00 USD sería 2000)
                    },
                    'quantity': 1,
                },
            ],
            mode="subscription",  # Usamos "subscription" para suscripciones
            metadata={"username": username},  # Puedes incluir el username como metadata
            success_url="https://example.com/success",  # URL que se redirige después de un pago exitoso
            cancel_url="https://example.com/cancel",  # URL en caso de que el usuario cancele el pago
        )
        ...
        # Crear la respuesta y guardar el username en la cookie
        response = jsonify(
            {
                "status": "success",
                "username": username,
...
async def stripe_webhook():
    ...
    try:
        if event["type"] == "checkout.session.completed":
            ...
            email = session.get("customer_email", "No email provided")  # Obtener el email de la sesión
            print(f"Pago completado por {username} - Email: {email}")

            # Aquí puedes actualizar la base de datos con el username y email si es necesario
            ...
            email = stripe_customer.email  # Obtener el email directamente del cliente
            print(f"USERNAME FROM WEBHOOK: {username} - Email: {email}")

            # Actualizar la base de datos: marcar como activo
...
# Función para actualizar el estado de la suscripción en la base de datos
async def update_user_subscription_status(username, status, period_end):
    db_connection = await get_db_connection()
    async with db_connection.cursor() as cursor:
        try:
            # Actualizar la base de datos con el nuevo estado de la suscripción
            await cursor.execute(
...
async def verify_subscription():
    ...
    if not username:
        return jsonify({"message": "Username no proporcionado"}), 400  # Si no hay username, error 400

    db_connection = await get_db_connection()
    async with db_connection.cursor(aiomysql.DictCursor) as cursor:
        try:
            # Consultar la base de datos para verificar la suscripción
...
async def login():
    ...
    if user:
        ...
        if bcrypt.checkpw(password.encode("utf-8"), user_dict["password"].encode("utf-8")):
            # Generar el token JWT con zona horaria UTC
            expiration_time = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=1)
            token = jwt.encode(
                {"username": username, "exp": expiration_time}, 
                SECRET_KEY, 
...
# Endpoint para actualizar detalles de cuenta MetaTrader 5
...
async def get_account_details(username):
    ...
    try:
        # Asegúrate de que la función get_db_connection esté bien definida para usarse con async
        conn = await get_db_connection()
...
# Inicialización de MetaTrader 5
...
# Endpoint para inicializar MetaTrader 5
...
async def initialize_mt5_endpoint():
    ...
    # Obtener detalles de la cuenta de manera asíncrona
    account_details = await get_account_details(username)  # Debes usar await aquí
    if not account_details:
...
def get_candle_data(symbol, timeframe, n_candles):
    ...
    if rates is None:
        clear_message()
        #print("No se pudieron obtener datos de velas, error:", mt5.last_error())
...
# Resto del código para la lógica del bot y los endpoints
bot_active = False
...
# Lista para almacenar los mensajes de las operaciones
operation_message = []  # Variable para almacenar el último mensaje
